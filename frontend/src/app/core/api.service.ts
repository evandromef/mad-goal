...
export interface Dashboard {
  ...
  positions: Position[];
  evolution: { period: string; acquisitionCost: number }[];
}
...
export class ApiService {
  ...
  createWallet(name: string): Observable<Wallet> {
    return this.http.post<Wallet>('/api/wallets', { name });
  }
  updateWallet(id: string, name: string): Observable<Wallet> {
    return this.http.put<Wallet>(`/api/wallets/${id}`, { name });
  }
