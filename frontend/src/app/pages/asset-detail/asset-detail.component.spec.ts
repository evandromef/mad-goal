...
describe('AssetDetailComponent', () => {
  ...
  beforeEach(async () => {
    ...
    api.createNote.mockReturnValue(of(note));
    api.updateNote.mockReturnValue(of(note));
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: ModalService, useValue: modal },
        {
          ...
          useValue: { snapshot: { paramMap: convertToParamMap({ walletId: 'w1', assetId: 'a1' }) } },
        },
      ],
    }).compileComponents();
  });
  ...
  it('filtra históricos e executa CRUD de notas', async () => {
    ...
    component.filter.set('OPERACOES');
    expect(component.filteredRecords()).toHaveLength(0);
    expect(component.historyValue(record)).toContain('10');
    expect(component.historyValue({ ...record, totalValue: undefined, newQuantity: 20, ratio: '1:2' })).toContain(
      '1:2',
    );
    component.noteForm.setValue({ content: 'Nova' });
    component.saveNote();
    expect(api.createNote).toHaveBeenCalled();
    component.editNote(note);
    component.noteForm.setValue({ content: 'Editada' });
    component.saveNote();
