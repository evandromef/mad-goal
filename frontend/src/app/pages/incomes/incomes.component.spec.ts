...
describe('IncomesComponent', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    api.assets.mockReturnValue(
      of([{ id: 'a1', ticker: 'PETR4', name: 'Petrobras', category: 'ACAO', currentPrice: 30, priceDate: null }]),
    );
    api.wallets.mockReturnValue(of([{ id: 'w1', name: 'Principal', currentValue: 100 }]));
    api.records.mockReturnValue(
      of([{ id: 'r1', walletId: 'w1', assetId: 'a1', ticker: 'PETR4', type: 'COMPRA', date: '2026-01-01' }]),
    );
    api.incomes.mockReturnValue(
      of({
        total: 25,
        groups: [{ period: '2026-01', total: 25 }],
        items: [
          {
            id: 'i1',
            assetId: 'a1',
  ...
  it('aplica filtros combinados', () => {
    ...
    expect(api.incomes).toHaveBeenLastCalledWith('w1', {
      assetId: 'a1',
      category: 'ACAO',
      type: 'DIVIDENDO',
      from: '2026-01-01',
