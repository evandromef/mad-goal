...
export class RecordsComponent implements OnInit {
  ...
  readonly recordAssets = computed(() =>
    Array.from(
  ...
  loadRecords(): void {
    this.api.records(this.walletId).subscribe((items) => this.records.set(items));
  }
  ...
  eventDetails(item: LedgerItem): string {
    if (item.newQuantity != null)
      return `${item.newQuantity.toLocaleString('pt-BR', { maximumFractionDigits: 8 })} un. · ${item.ratio ?? 'sem proporção'}`;
    return item.ratio ?? '—';
  }
}
